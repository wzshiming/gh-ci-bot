...
- The PR's merge commit is cherry-picked (with `-m 1` if it is a merge commit) onto a new branch named `cherry-pick-<pr-number>-to-<branch>`, which is pushed to the repository. For a PR merged with the rebase method, all of the PR's rebased commits are cherry-picked.

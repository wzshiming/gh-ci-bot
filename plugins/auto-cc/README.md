...
Requests reviews from automatically selected reviewers, using the same reviewer-selection logic as the automatic [blunderbuss](../blunderbuss/README.md) behavior that runs when a PR is opened.
...
- Reviewers are picked from the `OWNERS` files nearest to the changed files, falling back to the `REVIEWERS` environment variable; the PR author is never picked. See [blunderbuss](../blunderbuss/README.md) for the full selection logic.

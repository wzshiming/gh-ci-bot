...
begin_case "ensure-labels.sh: creates needs-rebase with prow's color and description"
mkrepolabels
run ensure-labels.sh "${NEEDS_REBASE_LABEL}"
assert_status 0
log_has "label -R wzshiming/example create ${NEEDS_REBASE_LABEL} --color e11d21"
log_has "Indicates a PR cannot be merged because it has merge conflicts with HEAD."

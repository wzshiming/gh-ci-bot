...
# gh pr edit --add-reviewer no longer fetches organizational teams when it's not necessary,
...
for reviewer in ${login}; do
  gh "${ISSUE_KIND}" -R "${GH_REPOSITORY}" edit "${ISSUE_NUMBER}" --add-reviewer "${reviewer}" ||
